...
import { RequestStatus, RequestSummary } from 'src/app/models/request';
...
@Component({
  ...
  providers: [{
    ...
    useFactory: (maintenanceService: MaintenanceService, inventoryService: InventoryService) => ofST(
      [],
      inventoryService.getConfig(KEY),
      server({
...
export class InventoryOutComponent implements OnInit, OnDestroy {
  ...
  getSummary() {
    ...
    if (this.dateForm.valid) {
      ...
      const typeId = this.inventoryService.getDefaultOutTypeId();
  ...
  resetFilter() {
    ...
    this.selectedStaff = this.sessionService.getUser();
    this.setUrlParams();

    sessionStorage.removeItem(KEY);

    this.maintenanceService.clearFilter(this._table, ['category_id', 'created_at', 'requested_at', 'facility_id', 'request_status_id', 'trip_id', 'room_id', 'status']);
    this.getSummary();
  }
